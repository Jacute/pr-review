...
<p align="center">
  ...
  <a href="#результаты-нагрузочного-тестирования">Результаты нагрузочного тестирования</a> •
...
Для тестирования использовался k6.

1. Тестирование создания команды POST /team/add и проверки, что она действительно создалась GET /team/get
Количество участников в команде случайное от 5 до 20.
![alt text](./docs/img/add_team_stress_test.png)

**RPS**: 3021
**SLI времени ответа** (среднее время выполнения запросов POST /team/add и GET /team/get вместе): 59.36ms
**SLI успешности**: 100%

Запуск:
```
make up
k6 run ./loadtests/add_team.js
```

2. Тестирование создания PR'ов
![alt text](./docs/img/pr_stress_test.png)

**RPS**: 4069
**SLI времени ответа**: 1.49ms
**SLI успешности**: 100%

Запуск:
```
make up
k6 run ./loadtests/pr.js
```
...
3. Добавлен эндпоинт со статистикой по количеству PR'ов от авторов GET /pullRequest/statistics с пагинацией
4. Добавлен [линтер](./.github/workflows/lint.yml) в ci
5. Проведено нагрузочное тестирование
6. Добавил валидацию в большинстве полей: стандартные проверки на пустые поля, длину, соответствие формату UUID и т.п.
